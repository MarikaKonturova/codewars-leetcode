...
function goodVsEvil(good, evil) {
  ...
  const goodList = ["hobbits", "men", "elves", "dwarves", "eagles", "wizards"];
  const evilList = [
    "orcs",
    "men",
    "wargs",
  ...
  const goodSum = good
    .split(" ")
    .reduce(
      (acc, curr, i) => acc + +curr * creatures["good_creatures"][goodList[i]],
      0
    );
  const evilSum = evil
    .split(" ")
    .reduce(
      (acc, curr, i) => acc + +curr * creatures["evil_creatures"][evilList[i]],
      0
    );
