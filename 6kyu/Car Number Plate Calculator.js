...
*/
function findTheNumberPlate1(customerID) {
  const alphabet = "abcdefghijklmnopqrstuvwxyz";

  const numeric = ("000" + ((customerID % 999) + 1).toString(10)).slice(-3);
  const l = Math.floor(customerID / 999);
  const alpha =
    alphabet[l % 26] +
    alphabet[Math.floor(l / 26) % 26] +
    alphabet[Math.floor(l / (26 * 26)) % 26];

  return alpha + numeric;
}

function findTheNumberPlate2(customerID) {
  ...
  let flooredID = Math.floor(customerID / 999);
  first = alphabet[flooredID % 26];
  second = alphabet[Math.floor(flooredID / 26) % 26];
  third = alphabet[Math.floor(Math.floor(flooredID / 26) / 26) % 26];

  return `${first}${second}${third}${numPart}`;
}

console.log(findTheNumberPlate2(21));
