...
decodeMorse = function (morseCode) {
  morseSentence = morseCode.split("   ");
  for (let i = 0; i < morseSentence.length; i++) {
    morseSentence[i] = morseSentence[i].trim().split(" ");
    for (let y = 0; y < morseSentence[i].length; y++) {
      morseSentence[i][y] = MORSE_CODE[morseSentence[i][y]];
    }
  }
